...
def initialize_workflow_node(state: WorkflowState, config: AppConfig) -> WorkflowState:
    """
    ...
    goal_root_path = Path(config.goal_root_path).resolve()
    workspace_root_path = Path(config.workspace_root_path).resolve()
    ...
    try:
        state['task_description_path'] = str(workspace_root_path / config.task_description_filename)
        state['manifest_template_path'] = str(workspace_root_path / config.manifest_template_filename)
        state['changelog_template_path'] = str(workspace_root_path / config.changelog_template_filename)
        
        state['manifest_output_path'] = str(goal_root_path / config.manifest_output_filename)
        state['changelog_output_path'] = str(goal_root_path / config.changelog_output_filename)

        log_subdirectory = goal_root_path / config.log_subdirectory_name
        ...
        state['overview_log_file_path'] = str(log_subdirectory / config.overview_log_filename)
        state['detailed_log_file_path'] = str(log_subdirectory / config.detailed_log_filename)
