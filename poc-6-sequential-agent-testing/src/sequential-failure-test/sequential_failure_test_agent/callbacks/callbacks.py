...
def _parse_outcome_json(outcome_json: Optional[str], state_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"Callback Info: Initial JSON parse failed for state key '{state_key}'. Raw content: '{outcome_json}'. Attempting to strip markdown fences.")
        ...
        try:
            ...
            print(f"Callback Error: Failed to parse JSON from state key '{state_key}' even after attempting to strip fences/quotes from: '{outcome_json}'")
